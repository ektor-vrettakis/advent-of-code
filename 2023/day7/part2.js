...
const order = 'J23456789TQKA'.split('')
  .reduce((acc, value, index) => ({ ...acc, [value]: index }), {});

const countRanks = hand => {
  const counts = hand.cards
    .reduce((acc, value) => ({ ...acc, [value]: (acc[value] | 0) + 1 }), {});
  if (counts['J']) {
    const [pair1, pair2] = Object.entries(counts)
      .sort(([_pair1, a], [_pair2, b]) => b - a)
      .map(pair => pair[0]);
    if (pair1 !== 'J')
      counts[pair1] += counts['J'];
    else if (pair2)
      counts[pair2] += counts['J'];
    else
      counts['A'] = 5;
    delete counts['J'];
  }
